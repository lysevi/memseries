    ...
    if [[ "$GCOV" == "TRUE" ]]; then
       ...
       cmake -DBoost_USE_STATIC_LIBS=ON -DENABLE_DOUBLECHECKS=ON -DBoost_USE_MULTITHREADED=ON  -DBoost_USE_STATIC_RUNTIME=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="${CMAKE_CXX_FLAGS_RELEASE} --coverage -O0" \
		...
		if [[ "${SANITIZER}" == "ASAN_UBSAN" ]]; then
			...
			cmake -DASAN_UBSAN=ON -DBoost_USE_STATIC_LIBS=ON -DENABLE_DOUBLECHECKS=ON -DBoost_USE_MULTITHREADED=ON  -DBoost_USE_STATIC_RUNTIME=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="${CMAKE_CXX_FLAGS_RELEASE}" \
			-DCMAKE_EXE_LINKER_FLAGS="${CMAKE_EXE_LINKER_FLAGS}"  .
		fi

		if [[ "${SANITIZER}" == "MSAN" ]]; then
		    echo "MSAN enabled"
			cmake -DMSAN=ON -DBoost_USE_STATIC_LIBS=ON -DENABLE_DOUBLECHECKS=ON -DBoost_USE_MULTITHREADED=ON  -DBoost_USE_STATIC_RUNTIME=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="${CMAKE_CXX_FLAGS_RELEASE}" \
			-DCMAKE_EXE_LINKER_FLAGS="${CMAKE_EXE_LINKER_FLAGS}"  .
		fi
